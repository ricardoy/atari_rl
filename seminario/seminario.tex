...
\frame {
  ...
  \begin{itemize}
    \item Repetição de experiências
...
\frame{
  ...
  Em \cite{dqn}, foi proposto um modelo de memória de repetição de
...
\frame{
  ...
  Para evitar oscilações, \cite{dqn2} propôs manter 2 parametrizações
  de rede neural em memória: $\theta$ e $\theta^{-}$.

  \begin{itemize}
  \item Os valores alvo do Q-Learning são calculados utilizando a parametrização fixa $\theta^{-}$:
    \begin{equation*}
      r + \gamma \max_{a'} Q(s', a', \theta^{-})
    \end{equation*}
  \item Em cada iteração do treinamento, a parametrização $\theta$ é
    atualizada.
  \item Periodicamente, atualizar a parametrização fixa: $\theta^{-}
    \leftarrow \theta$.
  \end{itemize}
...
\frame{
  ...
  A mesma idéia pode ser utilizada para cada camada da rede neural e
